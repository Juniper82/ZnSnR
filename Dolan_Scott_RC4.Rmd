...
```{r}
allOutcomes <- expand.grid(X1=0:1, X2=0:1, X3=0:1, X4=0:1, X5=0:1, X6=0:1)
allOutcomes
```
...
allOutcomes %>% 
  ...
  pmf.p <- table(pop)/length(table(rowSums(allOutcomes))) 
  pmf.p
```
...
```{r}
mu <- mean(pop)
sigma <- sqrt(var(pop)*((length(pop)-1)/length(pop)))
cat(paste0('The population mean is: ',mu))
cat(paste0('The population standard deviation is: ',sigma))
```
...
```{r}
set.seed(111)
pmf.s <- sample(0:6,size=1000,replace=T,prob=pmf.p)
```
...
```{r}
mu.s <- mean(pmf.s)
sigma.s <- sqrt(var(pmf.s)*((length(pmf.s)-1)/length(pmf.s)))
cat(paste0('The population mean is: ',mu.s))
cat(paste0('The population standard deviation is: ',sigma.s))
```
...
hist(pmf.s,freq=F)
```
>8. Use R to calculate the sample mean and sample standard deviation of your 1000 sampled “test scores.”

>9. Compare and contrast the Shape, Center and Spread of the pmf.p and the pmf.s.
